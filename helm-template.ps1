...
# # helm template (-f charts/values.prod.yaml)
# if ($args[0] -eq 'dev') {
#     helm template identityserver .\IdentityServer\charts\consumer-customers\ -n mrjamiebowman -f IdentityServer/charts/app-identityserver/values.dev.yaml --debug
# } elseif ($args[0] -eq 'prod') {
#     helm template identityserver .\IdentityServer\charts\consumer-customers\ -n mrjamiebowman -f IdentityServer/charts/app-identityserver/values.prod.yaml --debug
# } else {
#     helm template identityserver .\IdentityServer\charts\consumer-customers\ -n mrjamiebowman --debug
# }
